...
# if product_com.product_template_id is product_id:
	...
	def index(self, data_id=None, **post):
		...
		web_ref = request.env['website.best.sellers'].sudo().search([('id','=',data_id)],[])
		products_arr = [web.id for web in web_ref.product_id]
		_logger.warning(products_arr)
		products = request.env['product.template'].sudo().search([('id', 'in', products_arr)])
